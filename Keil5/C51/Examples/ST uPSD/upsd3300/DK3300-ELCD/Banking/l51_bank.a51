...
;------------------------------------------------------------------------------
; STMicroelectronics, Inc. -- Important Notes
; 
; ST Version 1.1, October 12, 2005
;
; Description - The original file (l51_bank.a51) distributed by Keil was 
; modified by ST for support of the uPSD33xx product in the banking example.
; Changes include flushing the Pre-Fetch Queue (PFQ) and Branch Cached (BC)
; when the code page is changed.  If the PFQ and BC are not flushed, they
; may contain values from a previously selected code page resulting in 
; invalid code execution.  The PFQ and BC are flushed when disabled and then
; enabled again.  The code doesn't check to see if the PFQ and BC are enabled
; before the disable/enable operation, so it is important to make appropriate
; changes if bank switching may be performed with the PFQ and BC turned off.
;
; Note: This code should be reviewed to ensure that the PFQ and BC are flushed,
;       as appropriate, for the type of banking used by the target application.
;
...
; INCIDENTAL OR CONSEQUENTIAL DAMAGES WITH RESPECT TO ANY CLAIMS ARISING IN 
; CONNECTION WITH OR ARISING FROM THE FURNISHING, PERFORMANCE, OR USE OF THIS 
; SOFTWARE.
;
...
;ST - PFQ and BC controls bits are in the BUSCON SFR
BUSCON  DATA    9DH
...
; <o> ?B_XDATAPORT: Page register offset based on CSIOP <0x0-0xFF>
;    <i> Specify the CSIOP based offset of the page register (usually E0H)
;?B_XDATAPORT    EQU     0FFFFH   ; default is XDATA Port Address 0FFFFH
;ST change																	  *
?B_XDATAPORT    EQU     PSD_REG_ADDR+0E0H   ; CSIOP addr+E0H = Page register  *
; <o> ?B_FIRSTBIT: Starting with Bit must be 0 for uPSD devices <0-7>
;     <i> Must be 0 for uPSD devices!
?B_FIRSTBIT     EQU     0         ; Must be 0 for uPSD devices!
;</h>
;-----------------------------------------------------------------------------
ENDIF;
;
...
IF ?B_NBANKS <= 16
  ;ST change - the value was increased from 16 to 32 to provide space for the 
  ;            added instructions to clear the uPSD33/34xx PFQ and BC.
  ; Convert Bank No in Accu to Address * 32
  IF  ?B_FIRSTBIT = 0
  CONVBANKNO    MACRO
                SWAP    A
                RL      A   ; ST change - line added
                ENDM
  ENDIF

  ;ST - all other CONVBANKNO macros are not relevant because ?B_FIRSTBIT must always be 0 with uPSD devices
  IF  ?B_FIRSTBIT = 1
  CONVBANKNO    MACRO
                SWAP    A
                RR      A
...
;ST addition - Clear uPSD33/34xx PFQ and BC (disable/enable to clear the PFQ and BC)
;
				ANL     BUSCON,#03Fh
				ORL     BUSCON,#0C0h
...
IF ?B_NBANKS <= 16
;               SWAP    A
;ST change - multiply by 32 instead of 16
                SWAP    A
                RL      A
ENDIF
                MOV     DPTR,#?BANK?SWITCH
